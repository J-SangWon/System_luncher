...
public class TitleManager : MonoBehaviour
{
    ...
    private IEnumerator LoadGameCo()
    {
        ...
        AudioManager.Instance.PlayBGM(BGM.lobby); // 로비 BGM 재생
        yield return new WaitForSeconds(5f); // BGM이 재생될 때까지 잠시 대기
        AudioManager.Instance.StopBGM(); // BGM 정지
        yield return new WaitForSeconds(1f); // 정지 후 잠시 대기
        AudioManager.Instance.ResumeBGM(); // BGM 재생 재개
        yield return new WaitForSeconds(5f); // 재생 재개 후 잠시 대기
        AudioManager.Instance.StopBGM(); // BGM 정지
